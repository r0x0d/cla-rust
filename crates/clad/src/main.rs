...
//! as a provider for Goose (https://github.com/block/goose) and other AI clients.
//!
...
//!    - backend.proxies: (Optional) HTTP/HTTPS proxy for outgoing backend requests
//!
//! 2. Build and run:
//!    $ cargo build --release
//!    $ cargo run --release
//!
//! 3. Configure Goose to use CLAD:
...
use crate::{
    config::Config,
    provider::{chat_completions_handler, health_check_handler, models_handler, create_authenticated_client},
    state::AppState,
};
